...
def build_exclusive_strength_geoms(
    ...
    *,
    ...
    if amber_outer is not None:
        amber_outer = _polygonal_geometry(amber_outer.intersection(red_outer))
    ...
    if green_outer is not None:
        ...
        green_outer = _polygonal_geometry(green_outer.intersection(green_limit))
    ...
    if amber_outer is not None:
        amber_only = _polygonal_geometry(
            amber_outer.difference(occupied)
            if occupied is not None
            else amber_outer
        )
        if amber_only is not None:
            exclusive_shapes['amber'] = amber_only
            occupied = unary_union((occupied, amber_only)) if occupied is not None else amber_only
    red_only = _polygonal_geometry(
        red_outer.difference(occupied)
        if occupied is not None
        else red_outer
    )
