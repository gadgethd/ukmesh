...
const ui = {
  ...
  signalQualityCard: document.querySelector('#signal-quality-card'),
  ...
  latencyCard: document.querySelector('#receipt-spread-card'),
...
function signalSummary(session) {
  ...
  if (points.length === 0) {
    return {
      value: 'No data yet',
      label: 'Waiting for a health check receipt.',
      tone: 'neutral',
      points: [],
      empty: true,
    };
...
function latencySummary(session) {
  ...
  if (receipts.length === 0) {
    return {
      value: 'No data yet',
      label: 'Waiting for observer reports.',
      tone: 'neutral',
      points: [],
      empty: true,
    };
...
function renderGlanceMetrics(session) {
  ...
  if (ui.signalQualityCard) {
    ui.signalQualityCard.classList.toggle('glance-card--empty', Boolean(signal.empty));
  }
  ...
  if (ui.latencyCard) {
    ui.latencyCard.classList.toggle('glance-card--empty', Boolean(latency.empty));
  }
  if (ui.latencyLabel) {
...
function applySiteBranding(snapshot) {
  ...
  if (externalUrl) {
    const externalActionLabel = /^open\b/i.test(externalLabel)
      ? externalLabel
      : `Open ${externalLabel}`;
    ui.externalLink.href = externalUrl;
    ui.externalLink.textContent = `${externalActionLabel} ↗`;
    ui.externalLink.setAttribute('aria-label', `${externalActionLabel} in a new tab`);
    ui.externalLink.classList.remove('hidden');
  } else {
    ui.externalLink.href = '#';
    ui.externalLink.textContent = 'External Link';
    ui.externalLink.removeAttribute('aria-label');
