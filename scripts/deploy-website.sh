...
DRIFT_OUT=$(scripts/check-website-drift.sh "$IMG" $( [ "$SVC" = "website-dev" ] && echo --staging ) 2>&1)
DRIFT_RC=$?
echo "$DRIFT_OUT"
...
docker compose -f docker-compose.yml -f docker-compose.live.yml up -d --no-deps "$SVC" || {
  echo "DEPLOY FAILED — restoring pin $OLD"; sed -i "s|^$PIN=.*|$PIN=$OLD|" .env; exit 1
}
...
[ "$PASS" = "1" ] && echo "VERIFIED: served bundles match $IMG on :$PORT"
