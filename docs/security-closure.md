...
| SEC-03 | Closed by removal | The viewshed producer and worker service are disabled, and the rejected coverage contracts return `410 Gone` without queue or database work. The retained link-v3 producer/consumer remains bounded and independently tested. |
...
| SEC-14 | Closed by removal | Planned-coverage POST/read/delete contracts return `410 Gone` and create no jobs. Dormant helpers and old data remain for one release only as an inactive whole-release rollback path. |
