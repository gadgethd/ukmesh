...
export function decryptOwnerSession(token: string): OwnerSession | null {
  try {
    ...
    // A username-bearing legacy cookie is accepted only as an identity hint.
    // Its embedded node IDs are ignored and it is reissued as v2 after current
    // server-side authorization succeeds.
