...
import { parseHexIdentifier } from '../utils/input.js';
import {
...
export function registerCoverageRoutes(router: Router, deps: CoverageRouteDeps): void {
  ...
  const coverageModelVersion = deps.coverageModelVersion
    ?? getCoverageModelVersion();
  ...
  router.get('/coverage', coverageLimiter, async (req, res) => {
    ...
    try {
      ...
      const modelVersionParam = `$${filters.params.length + 7}`;
      ...
      const result = await query<{ node_id: string }>(
        `SELECT nc.node_id, nc.geom, nc.strength_geoms, nc.antenna_height_m, nc.radius_m, nc.calculated_at
         ...
         WHERE (n.name IS NULL OR n.name NOT LIKE '%🚫%')
           AND n.role = 2
           AND n.lat BETWEEN ${minLatParam}::double precision - 2 AND ${maxLatParam}::double precision + 2
           AND n.lon BETWEEN ${minLonParam}::double precision - 2 AND ${maxLonParam}::double precision + 2
           AND nc.node_id > ${cursorParam}
           AND nc.model_version = ${modelVersionParam}
           AND nc.calculation_status = 'computed'
  ...
  router.get('/coverage/:nodeId', coverageLimiter, async (req, res) => {
    ...
    const nodeId = parseHexIdentifier(req.params.nodeId, {
      name: 'nodeId',
      minLength: 64,
      maxLength: 64,
    }).toUpperCase();
    try {
      ...
      const modelVersionParam = `$${filters.params.length + 2}`;
      ...
      const existing = await query<{
        ...
        `SELECT nc.node_id, nc.geom, nc.strength_geoms, nc.antenna_height_m, nc.radius_m, nc.calculated_at::text AS calculated_at
         ...
         WHERE nc.node_id = ${nodeIdParam}
           ...
           ${filters.nodesAlias('n')}
         LIMIT 1`,
        [...filters.params, nodeId, coverageModelVersion],
      );
      if (existing.rows[0]) {
