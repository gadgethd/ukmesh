...
// The UI only needs the newest sample per canonical node, so this cap cannot
// truncate a legitimate response for any currently supported network. It exists
// purely to bound the materialised DISTINCT ON result and JSON serialisation.
export const NODE_STATUS_LATEST_MAX_ROWS = 2_000;
const NODE_STATUS_LATEST_DEFAULT_TIMEOUT_MS = 8_000;
...
export type NodeStatusRouteDeps = {
  query: QueryFn;
  latestQueryTimeoutMs?: number;
};

export function registerNodeStatusRoutes(router: Router, deps: NodeStatusRouteDeps): void {
  router.get('/node-status/latest', async (req, res) => {
    const signal = AbortSignal.timeout(
      deps.latestQueryTimeoutMs ?? NODE_STATUS_LATEST_DEFAULT_TIMEOUT_MS,
    );
    try {
      ...
      const result = await deps.query<{
        ...
        `WITH identity_sources AS MATERIALIZED (
         ...
         SELECT * FROM latest
         ORDER BY time DESC
         LIMIT ${NODE_STATUS_LATEST_MAX_ROWS}`,
        params,
        signal,
      );
      ...
      if (signal.aborted) {
        console.error('[api] GET /node-status/latest timed out');
        res.status(503).json({ error: 'Node status query timed out', retryable: true });
        return;
      }
  ...
  router.get('/node-status/history', async (req, res) => {
    ...
    try {
      ...
      if (!nodeId) {
        ...
        const latestNode = await deps.query<{ node_id: string }>(
          `SELECT nss.node_id
...
const router = Router();
registerNodeStatusRoutes(router, { query });
